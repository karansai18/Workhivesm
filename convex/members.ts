...
export const remove = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const [messages, reactions, convOne, convTwo] = await Promise.all([
      ctx.db
        .query("messages")
      ...
      ctx.db
        .query("conversations")
        .withIndex("by_memberOneId", (q) => q.eq("memberOneId", member._id))
        .collect(),

      ctx.db
        .query("conversations")
        .withIndex("by_memberTwoId", (q) => q.eq("memberTwoId", member._id))
        .collect(),
    ]);

    const conversations = [...convOne, ...convTwo];
